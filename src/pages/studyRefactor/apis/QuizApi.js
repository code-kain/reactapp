// 퀴즈 API 담당: 퀴즈 목록, 문제 조회, 답안 제출 요청을 담당
...
export const fetchExperienceQuiz = async (quizType) => {
  ...
  if (!response.ok) throw new Error("체험 퀴즈 조회 실패");
...
export const fetchQuizQuestions = async (quizId) => {
  ...
  if (!response.ok) throw new Error("퀴즈 문제 조회 실패");
  const result = await response.json();
  if (!result.success) throw new Error(result.message);
  return result.data;
};
...
export const fetchQuizChoices = async (questionId) => {
  ...
  if (!response.ok) throw new Error("퀴즈 보기 조회 실패");
  const result = await response.json();
  if (!result.success) throw new Error(result.message);
  return result.data;
};
...
export const submitQuizAnswers = async ({ quizId, userId, answers }) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error("퀴즈 제출 실패");
    const result = await response.json();
    if (!result.success) throw new Error(result.message);
    return result.data;
  } finally {
